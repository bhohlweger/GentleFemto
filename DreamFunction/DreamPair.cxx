...
DreamPair::DreamPair(const char* name, float normleft, float normright) :
		fPair(nullptr), fPairShifted(), fPairRebinned(), fPairReweighted(), fPairUnfolded(), fFirstBin(
				-99), fNormLeft(normleft), fNormRight(normright), fName(name) {
}
...
int DreamPair::GetNDists() {
	...
	return 1 + fPairShifted.size() + fPairRebinned.size()
			+ fPairReweighted.size() + fPairUnfolded.size();
}
...
void DreamPair::FixShift(DreamDist* pair, DreamDist* otherDist, float kMin,
	...
	if (!fixedShift && ((fFirstBin == -99) || kMin == -99)) {
		std::cout << "Internal kStar=" << fFirstBin
				<< " and external one kStar=" << kMin
				<< ". Check if you ran ShiftForEmpty!" << std::endl;
	} else {
		...
		if (fFirstBin > kMin || fixedShift) {
			...
			MEShifted->Sumw2();
			const char* SEMultHistName = Form("%s_", SEMult->GetName());
			SEMultShifted = new TH2F(SEMultHistName, SEMultHistName, nBins,
					xMin, xMax, multBins, 1, multMax);
			SEMultShifted->Sumw2();
			const char* MEMultHistName = Form("%s_", MEMult->GetName());
			MEMultShifted = new TH2F(MEMultHistName, MEMultHistName, nBins,
					xMin, xMax, multBins, 1, multMax);
			MEMultShifted->Sumw2();
			...
			for (int iBin = startBin; iBin <= endBin; ++iBin) {
				if (iOtherBin < endOtherBin) {
					SEShifted->SetBinContent(iBin,
							SE->GetBinContent(iOtherBin));
					SEShifted->SetBinError(iBin, SE->GetBinError(iOtherBin));
					MEShifted->SetBinContent(iBin,
							ME->GetBinContent(iOtherBin));
					...
					for (int iMult = 1; iMult <= multBins; ++iMult) {
						...
						MEMultShifted->SetBinContent(iBin, iMult,
								MEMult->GetBinContent(iOtherBin, iMult));
...
//                << " in the case of " << SE->GetName() << std::endl;
		...
		for (int ikStar = 1; ikStar <= MEMult->GetNbinsX(); ++ikStar) {
			MEMultReweighted->SetBinContent(ikStar, iMult,
					MEMult->GetBinContent(ikStar, iMult) * weight);
			MEMultReweighted->SetBinError(ikStar, iMult,
					MEMult->GetBinError(ikStar, iMult) * weight);
		}
	}

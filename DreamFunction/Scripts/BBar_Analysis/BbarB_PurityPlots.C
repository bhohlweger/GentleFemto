...
std::cout << std::endl;
  ...
  auto* cProtonsPurity = new TCanvas("cProtonsPurity","cProtonsPurity",0,0,5000,3100);
  PurityProtons->SetTitle(" ; p_{T} [GeV/c]; Purity protons (%)");
  PurityProtons->GetXaxis()->SetRangeUser(0.5,4.05);
  PurityProtons->GetYaxis()->SetRangeUser(0.6,1.05);
  PurityProtons->SetLineColor(kBlack);
  PurityProtons->SetLineWidth(2);
  PurityProtons->SetMarkerStyle(kFullCircle);
  PurityProtons->SetMarkerSize(0.5);
  PurityProtons->SetMarkerColor(kBlack);
  PurityProtons->Draw();

  auto* cAntiProtonsPurity = new TCanvas("cAntiProtonsPurity","cAntiProtonsPurity",0,0,5000,3100);
  PurityAntiProtons->SetTitle(" ; p_{T} [GeV/c]; Purity Anti-protons (%)");
  PurityAntiProtons->GetXaxis()->SetRangeUser(0.5,4.05);
  PurityAntiProtons->GetYaxis()->SetRangeUser(0.6,1.05);
  PurityAntiProtons->SetTitle(" ; p_{T} [GeV/c]; Purity Anti-protons (%)");
  PurityAntiProtons->GetXaxis()->SetRangeUser(0.5,4.05);
  PurityAntiProtons->GetYaxis()->SetRangeUser(0.6,1.05);
  PurityAntiProtons->SetLineColor(kBlack);
  PurityAntiProtons->SetLineWidth(2);
  PurityAntiProtons->SetMarkerStyle(kFullCircle);
  PurityAntiProtons->SetMarkerColor(kBlack);
  PurityAntiProtons->SetMarkerSize(0.5);
  PurityAntiProtons->Draw();
  ...
  EfficiencyAntiLambda->GetXaxis()->SetRangeUser(0.5,10.);
   ...
   auto* cLambdaPurity = new TCanvas("cLambdaPurity","cLambdaPurity",0,0,5000,3100);
   PurityLambda->SetTitle(" ; p_{T} [GeV/c]; Purity #Lambda (%)");
   PurityLambda->GetXaxis()->SetRangeUser(0.5,8);
   PurityLambda->GetYaxis()->SetRangeUser(0.6,1.05);
   PurityLambda->SetLineColor(kRed+2);
   PurityLambda->SetLineWidth(2);
   PurityLambda->SetMarkerStyle(kFullCircle);
   PurityLambda->SetMarkerSize(0.5);
   PurityLambda->SetMarkerColor(kRed+2);
   PurityLambda->Draw();

   auto* cAntiLambdaPurity = new TCanvas("cAntiLambdaPurity","cAntiLambdaPurity",0,0,5000,3100);
   PurityAntiLambda->SetTitle(" ; p_{T} [GeV/c]; Purity Anti-#Lambda (%)");
   PurityAntiLambda->GetXaxis()->SetRangeUser(0.5,8);
   PurityAntiLambda->GetYaxis()->SetRangeUser(0.6,1.05);
   PurityAntiLambda->SetLineColor(kRed+2);
   PurityAntiLambda->SetLineWidth(2);
   PurityAntiLambda->SetMarkerStyle(kFullCircle);
   PurityAntiLambda->SetMarkerSize(0.5);
   PurityAntiLambda->SetMarkerColor(kRed+2);
   PurityAntiLambda->Draw();
  ...
  // Making PDF file
  gSystem->Exec(TString::Format("mkdir /Users/Valentina/cernbox/Analysis/BBbar/GentleFemto_Output/QA_%s/",prefix));
  TString foldername = TString::Format("/Users/Valentina/cernbox/Analysis/BBbar/GentleFemto_Output/QA_%s/",prefix);
  ...
  cProtonsPurity->SaveAs(foldername+"PurityProtons_full.pdf");
  cLambdaPurity->SaveAs(foldername+"PurityLambda_full.pdf");
  cAntiProtonsPurity->SaveAs(foldername+"PurityAntiProtons_full.pdf");
  cAntiLambdaPurity->SaveAs(foldername+"PurityAntiLambda_full.pdf");
}

...
void GetQADistributions(const char* PairName, DreamDist* PairOrg,
  ...
  TH1F* SEMultProjRaw = (TH1F*) SEMultRaw->ProjectionY("SEMultProjRaw",
                                                       SEMultRaw->FindBin(0.18),
                                                       SEMultRaw->FindBin(0.28));
  ...
  TH1F* MEMultProjRaw = (TH1F*) MEMultRaw->ProjectionY("MEMultProjRaw",
                                                       MEMultRaw->FindBin(0.18),
                                                       MEMultRaw->FindBin(0.28));
  ...
  TH1F* MEMultProjRew = (TH1F*) MEMultRew->ProjectionY("MEMultProjRew",
                                                       MEMultRew->FindBin(0.18),
                                                       MEMultRew->FindBin(0.28));
...
void ReweightingQA(TList* PairList) {
  ...
  for (int iQA = 0; iQA < nEntries; ++iQA) {
    ...
    PairRew->Calculate_CF(0.18, 0.28);
...
void METoSEReweightingBBar(const char* foldername) {
  const char* filenames[1] = { "LAL"};
  for (int iFile = 0; iFile < 1; ++iFile) {
    TString FileName = Form("%s/CFOutput_%s_8.root", foldername, filenames[iFile]);

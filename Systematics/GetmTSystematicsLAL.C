...
//  CATSinput->SetFixedkStarMinBin(true, 0.004);
  ...
  //Running systematics only on L-antiL:
  ...
  for (int iVar = 0; iVar < 45; ++iVar) {
    ...
    TString VarName = TString::Format("LALVar%u", iVar);
    DreamCF* CFLALVar = CATSinput->ObtainCFSystBBar(
        5, VarName.Data(), DreamVarFile->GetPairDistributions(2, 3, ""), nullptr);
    int femtoPairVar = CFLALVar->GetFemtoPairs(0, 0.2);
    ...
    mTLALDists->ObtainTheCorrelationFunctionBBar(
        gSystem->pwd(), prefix, TString::Format("LALVar%u", OutCounter));
    OutCounter++;
  }

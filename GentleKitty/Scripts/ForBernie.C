...
void RUN2_main(const unsigned& NumIter, const unsigned& NumJobs,
  ...
  if (system == 0) {  // pPb MB
    CalibBaseDir += "/home/hohlweger/cernbox/SystematicsAndCalib/pPbRun2_MB/";
...
//    vMod_pXim = 0;
    ...
    TFile* SystErrFile_pp =
        SystErrFileName_pp != "" ? new TFile(SystErrFileName_pp, "read") : NULL;
    ...
    if (OliFileName_pp == OliFileName_pL)
      ...
      OliFile_pL =
          OliFileName_pL != "" ? new TFile(OliFileName_pL, "read") : NULL;
    TH1F* OliHisto_pL =
        OliFile_pL ? (TH1F*) OliFile_pL->Get(HistRestName.Data()) : NULL;

    //!CHANGE PATH HERE
    TString SystErrFileName_pL = TString::Format("%s/C2totalsysPL.root",
                                                 CalibBaseDir.Data());
    TFile* SystErrFile_pL =
        SystErrFileName_pL != "" ? new TFile(SystErrFileName_pL, "read") : NULL;
    ...
    if (OliFileName_pp == OliFileName_LL)
      ...
      OliFile_LL =
          OliFileName_LL != "" ? new TFile(OliFileName_LL, "read") : NULL;
    TH1F* OliHisto_LL =
        OliFile_LL ? (TH1F*) OliFile_LL->Get(HistRestName.Data()) : NULL;

    //!CHANGE PATH HERE
    TString SystErrFileName_LL = TString::Format("%s/C2totalsysLL.root",
                                                 CalibBaseDir.Data());
    TFile* SystErrFile_LL =
        SystErrFileName_LL != "" ? new TFile(SystErrFileName_LL, "read") : NULL;
    ...
    TH1F* OliHisto_pXim =
        OliFile_pXim ?
            (TH1F*) OliFile_pXim->Get(Form("%s", HistRestName.Data())) :
            NULL;
    ...
    if (vSource == 0) {
      ...
      fitter->SetParameter("pp", DLM_Fitter1::p_sor0, GaussSourceSize / 1.4,
                           0.5, 1.6);
    } else if (vSource == 2) {
    ...
    if (FIX_CL)
      ...
      fitter->SetParameter("pLambda", DLM_Fitter1::p_Cl, -0.9, -1.2, -0.8);
    if (vMod_pL == 1) {
    ...
    if (vFit < 2)
      ...
      fitter->SetParameter("LambdaLambda", DLM_Fitter1::p_c, 0, -2e-4, 2e-4);
    if (vSource != 0)
    ...
    if (vSameRad == false) {
      if (vSource == 0) {
        ...
        fitter->SetParameter("pXim", DLM_Fitter1::p_sor0, GaussSourceSize / 1.4,
                             0.5, 1.6);
      } else if (vSource == 2) {
    ...
    if (FAST_PLOT) {
      ...
      if (vSource == 2) {
        ...
        info1->AddText(
            TString::Format("w=%.3f#pm%.3f",
                            ...
                            fitter->GetParError("pp", DLM_Fitter1::p_sor2)));
      }
      info1->AddText(
      ...
      info2->AddText(
          TString::Format("R(%s)=%.3f#pm%.3f", SOURCE_NAME.Data(),
                          fitter->GetParameter("pLambda", DLM_Fitter1::p_sor0),
                          fitter->GetParError("pLambda", DLM_Fitter1::p_sor0)));
      if (vSource == 2 && vSameRad == false) {
        info2->AddText(
            TString::Format(
                "R2=%.3f#pm%.3f",
                fitter->GetParameter("pLambda", DLM_Fitter1::p_sor1),
                fitter->GetParError("pLambda", DLM_Fitter1::p_sor1)));
        info2->AddText(
            TString::Format(
                "w=%.3f#pm%.3f",
                fitter->GetParameter("pLambda", DLM_Fitter1::p_sor2),
                fitter->GetParError("pLambda", DLM_Fitter1::p_sor2)));
      }
      info2->AddText(
          TString::Format("C_{l}=%.3f#pm%.3f",
      ...
      if (vSource == 0)
        info3->AddText(
            TString::Format(
                ...
                fitter->GetParameter("LambdaLambda", DLM_Fitter1::p_sor0),
                fitter->GetParError("LambdaLambda", DLM_Fitter1::p_sor0)));
      else
        info3->AddText("R(Gauss) fixed to 1.2");
      info3->AddText(TString::Format("a_{0}=%.2f#pm%.2f", a0_LL, a0_err_LL));
      info3->AddText(
          TString::Format("r_{eff}=%.2f#pm%.2f", reff_LL, reff_err_LL));
      info3->AddText(
          TString::Format(
              "C_{l}=%.2f#pm%.2f",
              fitter->GetParameter("LambdaLambda", DLM_Fitter1::p_Cl),
              fitter->GetParError("LambdaLambda", DLM_Fitter1::p_Cl)));
      info3->AddText(
          TString::Format("Local #chi^{2}/ndf=%.0f/%u=%.2f, p_{val}=%.3f",
                          Chi2_LL, EffNumBins_LL,
                          Chi2_LL / double(EffNumBins_LL),
      ...
      info3->AddText(
          TString::Format("STAR #chi^{2}/ndf=%.0f/%u=%.2f, p_{val}=%.3f",
      ...
      double pvalXiCoulomb_kSm60 = TMath::Prob(
          Chi2_pXim_COULOMB_kSm60, round(EffNumBins_pXim_COULOMB_kSm60));
      double nSigmaXiCoulomb_kSm60 = TMath::Sqrt(2)
          * TMath::ErfcInverse(pvalXiCoulomb_kSm60);

      double pvalXiCoulomb_exclPeak = TMath::Prob(
          Chi2_pXim_COULOMB_exclPeak, round(EffNumBins_pXim_COULOMB_exclPeak));

...
void testCats2() {
  TFile* myfile =
      TFile::Open(
          "~/cernbox/HM13TeV/AnalysisData/ClosePairRej/SelectedPairs/fOutput.root",
          "read");
...
//  pOmGauss->Draw("same");
  ...
  TLegend* leg = new TLegend(0.4, 0.65, 0.88, 0.87);
  leg->SetBorderSize(0);
...
//      "l");
  leg->Draw("same");
  TString outfile = TString::Format("%s/bautiful.root",gSystem->pwd());
  TFile* output = new TFile(outfile.Data(),
      "RECREATE");

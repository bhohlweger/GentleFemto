...
void GetXiForRadius(const unsigned& JobID, const unsigned& maxJobs,
  ...
  double Pars_pXi[6] = { 0, 0, 0, GaussSourceSize * 1.2, GaussSourceSize / 1.2,
      0.5 };
  TidyCats* tidy = new TidyCats();
  CATS AB_pXim;
  tidy->GetCatsProtonXiMinus(&AB_pXim, GaussSourceSize, Pars_pXi,
                             NumMomBins_pXim, kMin_pXim, kMax_pXim, true,
                             tQCDVars[tOut]);
  AB_pXim.KillTheCat();
...
//	fitter->FixParameter("pXiSideBand", DLM_Fitter1::p_pot3, SideBandPars[3]);
  ...
  std::cout << "Coulomb + Strong - pValue: " << pvalXi_exclPeak << "  sigma:  "
            << nSigmaXi_exclPeak << "  NDF:  " << EffNumBins_pXim_exclPeak
            << "  chi2:  " << Chi2_pXim_exclPeak << std::endl;
  ...
  TCanvas* cfast = new TCanvas(TString::Format("cfast_r%.2f", GaussSourceSize),
